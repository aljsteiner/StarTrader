 ...
 // A2Row[] entryGoods = {new A2Row(),new A2Row()}; // 
  ...
  ARow calcMoreManuals(int ix) {
    ...
    for (int m : E.alsecs) {
      ...
      ret.set(m, newKnowledge[mIx].get(m) * eM.newKnowledgeTradeManualFrac[0]
              + manuals[mIx].get(m) * eM.manualTradeManualFrac[0]
              + commonKnowledge[mIx].get(m) * eM.commonKnowledgeTradeManualFrac[0]
              + commonDif > 0. ? commonDif * eM.commonKnowledgeDifTradeManualFrac[0] : 0.
      );
    }
 ...
 */
  ...
  void accepted(Econ ec
    ...
    if (History.dl > 3) {
      ...
      hists[oIx].add(new History(5, cnName[myIx] + " c offer t=" + term, goodCargo));
      hists[myIx].add(new History(5, cnName[myIx] + " c offer t=" + term, goodCargo));
      hists[oIx].add(new History(5, cnName[myIx] + " g offer", goodGuests));
      hists[myIx].add(new History(5, cnName[myIx] + " g offer", goodGuests));

      hists[oIx].add(new History(5, cnName[1] + " old s cargo", cargos[1]));
      hists[myIx].add(new History(5, cnName[1] + " old s cargo", cargos[1]));
      hists[oIx].add(new History(5, cnName[1] + " old s guests", guests[1]));
      hists[myIx].add(new History(5, cnName[1] + " old s guests", guests[1]));
      hists[oIx].add(new History(5, cnName[1] + " old o cargo", cargos[0]));
      hists[myIx].add(new History(5, cnName[1] + " old o cargo", cargos[0]));
      hists[oIx].add(new History(5, cnName[1] + " old o guests", guests[0]));
      hists[myIx].add(new History(5, cnName[1] + " old o guests", guests[0]));
      hists[oIx].add(new History(5, cnName[1] + " old o manuals", manuals[0]));
      hists[myIx].add(new History(5, cnName[1] + " old o manuals", manuals[0]));
      hists[oIx].add(new History(5, cnName[1] + " old s manuals", manuals[1]));
      hists[myIx].add(new History(5, cnName[1] + " old s manuals", manuals[1]));
      hists[oIx].add(new History(5, cnName[1] + " old o moreManuals", moreManuals[0]));
      hists[myIx].add(new History(5, cnName[1] + " old o moreManuals", moreManuals[0]));
      hists[oIx].add(new History(5, cnName[1] + " old s moreManuals", moreManuals[1]));
      hists[myIx].add(new History(5, cnName[1] + " old s moreManuals", moreManuals[1]));
    }

    double t;
    // good is oriented to myIx  either P or S depending on the accept
    for (int m : E.alsecs) {
      double cbid = goodCargo.get(m);
      double gbid = goodGuests.get(m);
    // fix some possible fractional errors deriving cbid, gbid
      if (cbid < E.nzero) { // a request by myIx, offer by oix
        t= -cbid- cargos[oIx].get(m); 
