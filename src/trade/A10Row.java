...
//  int[] x2 = new int[E.l2secs];
  ...
  A10Row set(A10Row B) {
    ...
    double v=1.;
    for (int m : dA) {
      A[m].setCnt++; // flag change made in ARow
  ...
  public A10Row setDoNot(int group, int ixSrc, int srcIx, double nYears) {
    A[group * 2 + ixSrc].values[srcIx%LSECS] = nYears;
    dResums[0] = 99;
  ...
  A2Row filterByDoNot(int group, A2Row avail,double flag) {
    ...
    for (int m=0;m<2;m++) {
      for (int n=0;n< E.lsecs;n++) {
        ...
        if (A[m + 2 * group].values[n] > PZERO) {
          rtn.A[m].values[n] = flag;
         // rtn.A[m].setCnt++;
        }
      }
    }
  ...
  A2Row filterByDoNot(int group,int row, A2Row avail,double flag) {
    ...
    for (int m=0;m<2;m++) {
      for (int n=0;n< E.lsecs;n++) {
        // if doNot.get > 0, substitute flag value
        if (A[row + 2 * group].values[n] > PZERO) {
          rtn.A[m].values[n] = flag;
          rtn.A[m].setCnt++;  // force resetting of maxIx for n
        }
      }
    }
    return rtn;
