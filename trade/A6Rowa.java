...
/** get the grades reference for Staff
     ...
     public A6Rowa copyValues(A6Rowa prev){
       ...
       for(int m=0; m < len;m++){
         ...
         for(int n=0;n< E.LSECS;n++){
           if(E.debugDouble){
            A[m].values[n] = doubleTrouble(prev.A[m].values[n]);
           } else {
  ...
  void makeOrderIx(int[] ix, int x, double[] sum, int rowIx1, int rowIx2) {
    ...
    double minC, minO;
    int minOIx, minCIx;
    aCnt[rowIx1] = A[rowIx1].setCnt;
    aCnt[rowIx2] = A[rowIx2].setCnt;
    minSum[x] = minSum2[x] = negSum[x] = plusSum[x] = sum[x] = 0.;
    for (int g = 0; g < L2SECS; g++) {
      if (g < LSECS) {
        minC = A[rowIx1].values[g];
      }
      else {
        minC = A[rowIx2].values[g - LSECS];
      }
      sum[x] += minC;
      if (minC < NZERO) {
        negSum[x] += minC;
...
//    System.out.println("into resum title=" + titl + " la=" + lA + ", length A=" + A.length + ", m=" + m);
  ...
  void checkIx(int x) {
    ...
    resum(1 + x * 2);
    if (aCnt[x * 2] != A[x * 2].setCnt || aCnt[1 + x * 2] != A[1 + x * 2].setCnt) {
